...
- `Reference.confidence` is an optional `f64` carried on edges that have been resolved through a file-local symbol/import table. Tiers: `1.0` = same-file bare-identifier call (the caller and callee are both in this file's symbol set); `0.8` = call resolved via a file-local import alias to a fully-qualified path, with a second edge emitted in `<import-path>/<rest>` form (e.g. Go `fmt.Println` → `fmt/Println`, Python `np.array` after `import numpy as np` → `numpy/array`, Rust `Baz::greet` after `use foo::Bar as Baz;` → `foo::Bar/greet`); `None` = unresolved bare textual reference. Tier-2 ships for Go, Java, Kotlin, PHP, Python, JS, TS, Scala, and Rust; C# (`using F = Ns.T;` aliased form) is deferred pending tree-sitter-c-sharp grammar-shape work.
