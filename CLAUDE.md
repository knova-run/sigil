...
- Kotlin FQN imports: `resolve_kotlin_fqn_imports` uses Kotlin's standard `<root>/<pkg-as-dirs>/<File>.kt` layout to disambiguate same-named symbols across packages. For each bare `.kt` call ref, scan the file's `import com.x.y.<name>` entries; when exactly one file under `*/com/x/y/` defines the callable, emit a 0.7 file-resolved edge. Path-based — no `settings.gradle(.kts)` parsing required for the standard Gradle layout; non-standard `srcDirs(...)` overrides are a follow-up.
